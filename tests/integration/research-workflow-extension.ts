...
function financialFixture(): FinancialDataPlugin {
  const rows: NormalizedFinancialRow[] = [{
    ...
    plugin: 'fixture-financial', source: 'financial-fixture', retrievedAt: '2026-08-24T00:00:00.000Z', quality: 'high', confidence: 0.9,
  }]
  const data = buildFinancialData(rows)
  return {
    name: 'fixture-financial',
    async fetch() { return { data, metadata: { plugin: 'fixture-financial', source: 'financial-fixture', timestamp: '2026-08-24T00:00:00.000Z', quality: 'high' as const, confidence: 0.9 } } },
...
export async function apply(ctx: Context, config: Config): Promise<void> {
  ...
  const announcement = registerAnnouncementPlugin(plugins.registry, {
    sourceAdapter: {
  ...
  const media = registerMediaPlugin(plugins.registry, {
    sourceAdapter: {
  ...
  const financial = plugins.registry.register(financialFixture())
  const eventWorkflow = new EventAnalysisWorkflow({
    marketPlugin: new MarketPlugin(plugins.registry, plugins.market),
    newsPlugin: new NewsPlugin(plugins.registry, announcement.news),
    announcementPlugin: new NewsPlugin(plugins.registry, announcement.news),
    mediaPlugin: new NewsPlugin(plugins.registry, media.news),
    financialPlugin: new FinancialPlugin(plugins.registry, financial),
