...
import {
  ...
  registerAnnouncementProvider,
} from '../../packages/providers/adapters/announcement/index.ts'

test('Event Analysis consumes Announcement Provider output and creates research artifacts', async () => {
  const registry = new ProviderRegistry()
  const market = registry.register(new MockMarketProvider())
  const announcement = registerAnnouncementProvider(registry, {
    sourceAdapter: {
  ...
  const workflow = new EventAnalysisWorkflow({
    ...
    newsCapability: new NewsCapability(registry, announcement.news),
    artifactIdFactory: (type, ordinal) => `${type}-${ordinal}`,
  })

  const result = await workflow.run({
    symbol: '600519',
    sessionId: 'announcement-provider-session',
