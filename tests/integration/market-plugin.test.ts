...
import extension from './market-plugin-extension.ts'
...
test('Market Plugin Tool delegates to Mock Plugin and persists its Session result', async () => {
  const ctx = new Context()
  const sessionRoot = await mkdtemp(join(tmpdir(), 'researchhub-market-session-'))

  try {
    await mountAgentLoopTestDependencies(ctx, {
      systemPrompt: { persona: 'ResearchHub market plugin validation agent.' },
    })
    ...
    await handle.agent.followup({
      id: MessageId('researchhub-market-plugin-prompt'),
      role: 'user',
    ...
    const finalAssistant = events.find(event => event.type === 'assistant/message' && event.data.message.content.some(block => block.type === 'text' && block.text.includes('Market plugin integration validation completed')))
    ...
    assert.match(persisted, /Market plugin integration validation completed/)
