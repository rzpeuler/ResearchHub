...
export const name = 'researchhub-market-plugin-validation-extension'
...
class MarketPluginMockAdapter extends LlmAdapter {
  private readonly script = [
    toolCallResponse('get_market_snapshot', '{"symbol":"600519"}', 'market-snapshot-call'),
    textResponse('Market plugin integration validation completed'),
  ]
  ...
  async *stream(_options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (response === undefined) throw new Error('market plugin mock response script exhausted')
    yield* response
  }
}
...
export async function apply(ctx: Context): Promise<void> {
  ...
  const plugin = new MarketPlugin(plugins.registry, plugins.market)
  registerMarketPluginTool(ctx, plugin)
  new ResearchManager(ctx)
  ctx.llm.registerAdapter(['researchhub-market-validation'], new MarketPluginMockAdapter())
}
