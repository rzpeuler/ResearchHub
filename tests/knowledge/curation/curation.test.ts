...
function errorCode(action: () => Promise<unknown>, code: string): Promise<void> { return assert.rejects(action, (error: unknown) => error instanceof KnowledgeCurationError && (error.code === code || (error.code === 'candidate_set_exhausted' && error.candidateValidationRejections?.some((rejection) => rejection.code === code)))) as Promise<void> }
...
test('extractKnowledge retry preserves the C8 projection and adds only bounded validation feedback', async () => {
  const input = extractionProjectionInput()
  const invalid = { entities: [], relations: 'not-an-array', claims: [] }
  ...
  await assert.rejects(skill.extractKnowledge(input), (error: unknown) => error instanceof KnowledgeCurationError && error.code === 'invalid_model_output')
...
test('relation endpoint diagnostics are schema-derived and identify the failing candidate', async () => { const { skill } = makeSkill({ entities: [], relations: [supplierRelation('industry', 'company')], claims: [] }, 'extractKnowledge'); let error: KnowledgeCurationError | undefined; await assert.rejects(() => skill.extractKnowledge({ ...scope, batch: { batchId: 'batch-0001', sections: [], chunks: document.chunks }, reportUnderstanding: understanding() as never, knowledgeContext: context }), (value: unknown) => { if (value instanceof KnowledgeCurationError) error = value; return value instanceof KnowledgeCurationError && value.code === 'candidate_set_exhausted' }); assert.ok(error); assert.equal(error.candidateValidationRejections?.[0]?.code, 'invalid_semantics'); assert.equal(error.candidateValidationRejections?.[0]?.originalOrdinal, 1); assert.equal(error.candidateValidationRejections?.[0]?.relationType, 'supplier_of') })
...
test('business_exposure diagnostics report received and allowed endpoint types', async () => { const invalid = relation(); invalid.sourceMention = { text: 'Company', entityType: 'industry', existingRef: null }; invalid.targetMention = { text: 'Industry', entityType: 'industry', existingRef: null }; const { skill } = makeSkill({ entities: [], relations: [invalid], claims: [] }, 'extractKnowledge'); let error: KnowledgeCurationError | undefined; await assert.rejects(() => skill.extractKnowledge({ ...scope, batch: { batchId: 'batch-0001', sections: [], chunks: document.chunks }, reportUnderstanding: understanding() as never, knowledgeContext: context }), (value: unknown) => { if (value instanceof KnowledgeCurationError) error = value; return value instanceof KnowledgeCurationError && value.code === 'candidate_set_exhausted' }); assert.ok(error); assert.equal(error.candidateValidationRejections?.[0]?.code, 'invalid_semantics'); assert.equal(error.candidateValidationRejections?.[0]?.relationType, 'business_exposure') })
test('same-type relation diagnostics preserve the frozen constraint', async () => { const { skill } = makeSkill({ entities: [], relations: [substitutesRelation()], claims: [] }, 'extractKnowledge'); let error: KnowledgeCurationError | undefined; await assert.rejects(() => skill.extractKnowledge({ ...scope, batch: { batchId: 'batch-0001', sections: [], chunks: document.chunks }, reportUnderstanding: understanding() as never, knowledgeContext: context }), (value: unknown) => { if (value instanceof KnowledgeCurationError) error = value; return value instanceof KnowledgeCurationError && value.code === 'candidate_set_exhausted' }); assert.ok(error); assert.equal(error.candidateValidationRejections?.[0]?.code, 'invalid_semantics'); assert.equal(error.candidateValidationRejections?.[0]?.relationType, 'substitutes_for') })
