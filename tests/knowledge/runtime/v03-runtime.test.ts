...
test('v0.3 ChangeSet invalid matrix rejects canonical and semantic violations', async () => {
  ...
  try {
    ...
    const cases: Array<{ name: string; changeSet: KnowledgeChangeSetV03; code: string }> = [
      ...
      { name: 'claim confidence', changeSet: { ...base('confidence'), knowledgeOperations: [{ operationId: 'invalid-confidence', type: 'create', object: { id: 'claim:invalid-confidence', claimType: 'fact', statement: 'Invalid confidence', subjectRefs: ['entity:example-company'], sourceRefs: ['source:report'], confidence: Infinity, lifecycle: { status: 'active' } } as never }] }, code: 'V03_NUMERIC_CONSTRAINT' },
      { name: 'claim temporal', changeSet: { ...base('temporal'), knowledgeOperations: [{ operationId: 'invalid-temporal', type: 'create', object: { id: 'claim:invalid-temporal', claimType: 'fact', statement: 'Invalid temporal claim', subjectRefs: ['entity:example-company'], sourceRefs: ['source:report'], temporal: 'not-an-object' } as never }] }, code: 'V03_TEMPORAL_INVALID' },
      { name: 'raw context reference', changeSet: { ...base('context'), knowledgeOperations: [{ operationId: 'invalid-context', type: 'create', object: { id: 'relation:invalid-context', type: 'upstream_of', sourceRef: 'entity:semiconductor', targetRef: 'entity:semiconductor', sourceRefs: ['source:report'], contextRefs: [`raw-sha256-${'f'.repeat(64)}`], lifecycle: { status: 'active' } } as never }] }, code: 'V03_CONTEXT_REF_INVALID' },
      { name: 'financial contribution', changeSet: { ...base('financial'), knowledgeOperations: [{ operationId: 'invalid-financial', type: 'create', object: { id: 'relation:invalid-financial', type: 'business_exposure', sourceRef: 'entity:example-company', targetRef: 'entity:semiconductor', sourceRefs: ['source:report'], lifecycle: { status: 'active' }, attributes: { financialContribution: { revenueAmount: '1' } } } as never }] }, code: 'V03_RELATION_ATTRIBUTE_INVALID' },
      { name: 'module rows', changeSet: { ...base('module'), knowledgeOperations: [{ operationId: 'invalid-module', type: 'create', object: { id: 'module:invalid-module', type: 'market', columns: { invalid: true } } as never }] }, code: 'V03_FIELD_TYPE' },
...
test('v0.3 ChangeSet enforces requiresRawProvenance policy and virtual Raw context', async () => {
  const root = await createRoot()
  try {
    const registry = writableRegistry(); const loader = new KnowledgeBaseLoader({ registry }); const handle = await loader.mount(root); const validationSkill = new KnowledgeValidationSkill({ loader }); const source = (await loader.loadRuntimeState(handle)).index.getSource('source:report')
    const sourceCreate = (id: string, requiresRawProvenance: boolean): KnowledgeChangeSetV03 => ({ changeSetId: `changeset-provenance-${id.split(':')[1]}`, workflowRunId: `workflow-provenance-${id.split(':')[1]}`, knowledgeBaseId: handle.knowledgeBaseId, schemaVersion: '0.3', storageFormatVersion: '1', expectedBaseRevision: 0, requiresRawProvenance, sourceOperations: [{ operationId: `create-${id.split(':')[1]}`, type: 'source_create', source: { id, title: 'Policy Source', sourceType: 'community' } }], knowledgeOperations: [] })
    const missing = await validationSkill.validateChangeSet(handle, sourceCreate('source:policy-missing', true)); assert.equal(missing.report.errors.some((error) => error.code === 'V03_RAW_PROVENANCE_REQUIRED'), true); assert.equal(missing.validatedChangeSet, undefined)
...
test('v0.3 Relation contextRefs require existing Raw identities', async () => {
  const root = await createRoot()
  try {
    const registry = writableRegistry(); const loader = new KnowledgeBaseLoader({ registry }); const handle = await loader.mount(root); const validationSkill = new KnowledgeValidationSkill({ loader }); const raw = await archiveRaw(handle, { bytes: Buffer.from('context-raw'), originalFilename: 'context.txt' }); const relation = { id: 'relation:raw-context', type: 'upstream_of', sourceRef: 'entity:semiconductor', targetRef: 'entity:semiconductor', sourceRefs: ['source:report'], contextRefs: [raw.manifest.rawRef], lifecycle: { status: 'active' } }; const valid: KnowledgeChangeSetV03 = { changeSetId: 'changeset-raw-context-valid', workflowRunId: 'workflow-raw-context-valid', knowledgeBaseId: handle.knowledgeBaseId, schemaVersion: '0.3', storageFormatVersion: '1', expectedBaseRevision: 0, requiresRawProvenance: false, sourceOperations: [], knowledgeOperations: [{ operationId: 'create-raw-context', type: 'create', object: relation as never }] }; const validResult = await validationSkill.validateChangeSet(handle, valid); assert.equal(validResult.report.status, 'passed', JSON.stringify(validResult.report.errors))
    const invalid = { ...valid, changeSetId: 'changeset-raw-context-invalid', workflowRunId: 'workflow-raw-context-invalid', knowledgeOperations: [{ operationId: 'create-invalid-raw-context', type: 'create', object: { ...relation, id: 'relation:raw-context-invalid', contextRefs: [`raw-sha256-${'b'.repeat(64)}`] } as never }] }; const invalidResult = await validationSkill.validateChangeSet(handle, invalid); assert.equal(invalidResult.report.errors.some((error) => error.code === 'V03_CONTEXT_REF_INVALID'), true); assert.equal(invalidResult.validatedChangeSet, undefined)
  } finally { await rm(root, { recursive: true, force: true }) }
})
