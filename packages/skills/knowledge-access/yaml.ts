...
class Parser {
  ...
  private parseBlock(indent: number): unknown {
    ...
    if (!line || line.indent !== indent) {
      throw new KnowledgeError('ParseError', `Unexpected YAML indentation near line ${line?.lineNumber ?? 'end'}`)
    }
  ...
  private parseSequence(indent: number): unknown[] {
    ...
    while (this.index < this.lines.length) {
      ...
      if (next && next.indent > indent) {
        const nested = this.parseMapping(next.indent)
...
export function parseYaml(text: string, filePath = '<memory>'): unknown {
  ...
  try {
    ...
    if (error instanceof KnowledgeError) {
      throw new KnowledgeError(error.code, error.message, filePath)
    }
