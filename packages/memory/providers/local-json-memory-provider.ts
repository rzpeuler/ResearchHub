...
import type { MemoryEntry, MemoryEntryPatch, MemoryProvider, MemoryQuery } from '../core/index.ts';
...
/** JSON-file implementation of the MemoryProvider contract. */
export class LocalJsonMemoryProvider implements MemoryProvider {
  ...
  private enqueue<T>(operation: () => Promise<T>): Promise<T> {
    let queue = LocalJsonMemoryProvider.operationQueues.get(this.filePath);
    if (queue === undefined) {
      queue = { tail: Promise.resolve(), pending: 0 };
      LocalJsonMemoryProvider.operationQueues.set(this.filePath, queue);
    }
  ...
  private releaseQueue(queue: OperationQueueState): void {
    queue.pending -= 1;
    if (queue.pending === 0 && LocalJsonMemoryProvider.operationQueues.get(this.filePath) === queue) {
      LocalJsonMemoryProvider.operationQueues.delete(this.filePath);
    }
  }
  ...
  private async persistEntries(entries: readonly MemoryEntry[]): Promise<void> {
    ...
    const temporaryPath = `${this.filePath}.${process.pid}.${Date.now()}.${LocalJsonMemoryProvider.temporaryFileCounter++}.tmp`;
