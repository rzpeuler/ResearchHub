...
export const newsSearchDefinition = definePlugin({
...
export type NewsSearchInput = PluginInput<typeof newsSearchDefinition>
export type NewsSearchResult = PluginOutput<typeof newsSearchDefinition>
...
export interface NewsPluginData {
  symbol: string
  items: NewsPluginItem[]
}

export type NewsPluginHandle = PluginHandle<NewsSearchInput, NewsPluginData>
...
export class NewsPlugin {
  ...
  constructor(
    ...
    private readonly pluginHandle: NewsPluginHandle,
  ) {}
...
function validateNewsSearchResult(value: unknown, expectedSymbol: string): asserts value is NewsSearchResult {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new TypeError('search_company_news Plugin result must be an object')
  }

  const result = value as Record<string, unknown>
  assertAllowedFields(result, new Set(['symbol', 'items']))
  if (result.symbol !== expectedSymbol) {
    throw new TypeError('search_company_news Plugin result symbol must match the request')
  }
  if (!Array.isArray(result.items)) {
    throw new TypeError('search_company_news Plugin result items must be an array')
  }

  for (const [index, item] of result.items.entries()) {
    if (item === null || typeof item !== 'object' || Array.isArray(item)) {
      throw new TypeError(`search_company_news Plugin item ${index} must be an object`)
    }
    ...
    if (evidence.symbol !== expectedSymbol) {
      throw new TypeError(`search_company_news Plugin item ${index} symbol must match the request`)
    }
