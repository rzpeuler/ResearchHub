...
import { FINANCIAL_DATA_QUALITIES, type DataPlugin, type FinancialDataMetadata, type FinancialDataQuality, type JsonObject, type JsonValue, type PluginResult } from './types.ts'
...
function assertQuality(value: unknown, path: string): asserts value is FinancialDataQuality {
  if (!isFinancialDataQuality(value)) {
    throw new PluginValidationError('expected one of: high, medium, low', path)
  }
}

function assertConfidence(value: unknown, path: string): asserts value is number {
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || value > 1) {
    throw new PluginValidationError('expected a finite number between 0 and 1', path)
  }
}
...
/** Validates the metadata attached to a Plugin result. */
export function validateFinancialDataMetadata(value: unknown): asserts value is FinancialDataMetadata {
  if (!isPlainObject(value)) {
    throw new PluginValidationError('expected a plain object')
  }

  if (!isJsonObject(value)) {
    throw new PluginValidationError('expected a JSON-safe object')
  }

  assertAllowedFields(value, new Set(['plugin', 'source', 'timestamp', 'quality', 'confidence']))
  if (!Object.prototype.hasOwnProperty.call(value, 'plugin')) {
    throw new PluginValidationError('missing required field', '$.plugin')
  }
  if (!Object.prototype.hasOwnProperty.call(value, 'source')) {
    throw new PluginValidationError('missing required field', '$.source')
  }
  if (!Object.prototype.hasOwnProperty.call(value, 'timestamp')) {
    throw new PluginValidationError('missing required field', '$.timestamp')
  }
  if (!Object.prototype.hasOwnProperty.call(value, 'quality')) {
    throw new PluginValidationError('missing required field', '$.quality')
  }
  if (!Object.prototype.hasOwnProperty.call(value, 'confidence')) {
    throw new PluginValidationError('missing required field', '$.confidence')
  }

  assertNonEmptyString(value.plugin, '$.plugin')
...
/** Validates the JSON-safe envelope returned by a DataPlugin. */
export function validatePluginResult(value: unknown): asserts value is PluginResult<JsonValue> {
  if (!isPlainObject(value)) {
    throw new PluginValidationError('expected a plain object')
  }
  ...
  if (!Object.prototype.hasOwnProperty.call(value, 'data')) {
    throw new PluginValidationError('missing required field', '$.data')
  }
  if (!Object.prototype.hasOwnProperty.call(value, 'metadata')) {
    throw new PluginValidationError('missing required field', '$.metadata')
  }

  validateFinancialDataMetadata(value.metadata)
}

/** Validates the callable shape and stable name of a Plugin before registration. */
export interface DataPluginInspection {
...
/** Reads and validates a Plugin's callable boundary exactly once. */
export function inspectDataPlugin(value: unknown): DataPluginInspection {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw new PluginValidationError('expected a Plugin object')
  }
  ...
  try {
    ...
    throw new PluginValidationError('Plugin properties could not be read')
  }

  assertNonEmptyString(name, '$.name')
  if (typeof fetch !== 'function') {
    throw new PluginValidationError('expected a fetch function', '$.fetch')
  }
  if (typeof validate !== 'function') {
    throw new PluginValidationError('expected a validate function', '$.validate')
  }

  return {
    plugin: value as DataPlugin<unknown, unknown>,
    name,
    fetch: fetch as DataPlugin<unknown, unknown>['fetch'],
    validate: validate as DataPlugin<unknown, unknown>['validate'],
  }
}

export function validateDataPlugin(value: unknown): asserts value is DataPlugin<unknown, unknown> {
  inspectDataPlugin(value)
}

/** Validates a Plugin name used by Registry operations. */
export function assertPluginName(value: unknown, path = '$.name'): asserts value is string {
