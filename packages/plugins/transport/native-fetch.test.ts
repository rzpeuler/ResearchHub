...
test('native fetch transport delegates requests to an injected implementation', async () => {
  ...
  const result = await transport.request('https://plugin.example.test/quote', init)

  assert.equal(receivedInput, 'https://plugin.example.test/quote')
...
test('native fetch transport propagates injected transport failures', async () => {
  ...
  await assert.rejects(transport.request('https://plugin.example.test/quote'), (error: unknown) => error === failure)
})
