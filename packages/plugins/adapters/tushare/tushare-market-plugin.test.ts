...
import { TushareMarketPlugin } from './tushare-market-plugin.ts'
...
test('Tushare normalizes a documented daily response and sends a POST request', async () => {
  ...
  const plugin = new TushareMarketPlugin({
...
test('Tushare endpoint errors reject userinfo without exposing endpoint credentials or token', async () => {
  ...
  assert.throws(() => new TushareMarketPlugin({ endpoint, token }), (error: unknown) => {
...
test('Tushare validates normalized Market data through the DataPlugin contract', () => {
  const plugin = new TushareMarketPlugin({ token: 'fixture-token' })
  assert.doesNotThrow(() => plugin.validate({
    symbol: '600519',
    price: 1,
    change: 0,
    volume: 1,
  ...
  assert.throws(() => plugin.validate({ symbol: '600519' }), /expected a finite number/)
})
