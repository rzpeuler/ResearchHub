...
/** AKShare Bridge adapter; the external Bridge owns the Python SDK runtime. */
export class AkShareFinancialPlugin implements FinancialDataPlugin {
  ...
  async fetch(request: Parameters<FinancialDataPlugin['fetch']>[0]) {
    ...
    const rows = normalized.statementTypes
      .map(statementType => normalizedRows.find(row => row.statementType === statementType))
      .filter((row): row is NormalizedFinancialRow => row !== undefined)
