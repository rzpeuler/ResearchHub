...
import {
  ...
  type AnnouncementPluginData,
  type AnnouncementPluginRequest,
...
/** Canonical Information Layer Plugin for official company announcements. */
export class AnnouncementPlugin implements DataPlugin<AnnouncementPluginRequest, AnnouncementPluginData> {
  ...
  constructor(options: AnnouncementPluginOptions) {
  ...
  async fetch(request: AnnouncementPluginRequest) {
    ...
    if (!(timestamp instanceof Date) || Number.isNaN(timestamp.getTime())) {
      throw new AnnouncementPluginError('announcement plugin clock returned an invalid date')
    }
  ...
  validate(value: unknown): asserts value is AnnouncementPluginData {
    validateAnnouncementPluginData(value)
  }
}

export function normalizeAnnouncementRequest(value: AnnouncementPluginRequest): {
  symbol: string
  limit: number
} {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new PluginValidationError('expected an Announcement plugin request')
  }
...
function normalizeAnnouncementRecord(
  ...
  if (!Number.isFinite(confidence) || confidence < 0 || confidence > 1) {
    throw new AnnouncementPluginError('announcement confidence must be between 0 and 1')
  }
...
function mapSymbols(
  ...
  if (mappedSymbol === undefined) {
    throw new AnnouncementPluginError('announcement record cannot be mapped to a stock symbol')
  }
  if (mappedSymbol !== requestedSymbol) {
    throw new AnnouncementPluginError('announcement source symbol does not match the request')
  }
  return [mappedSymbol]
}
...
function averageConfidence(items: AnnouncementPluginData['items']): number {
  return Number((items.reduce((total, item) => total + item.confidence, 0) / items.length).toFixed(4))
}

function requireText(value: string, field: string): string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new AnnouncementPluginError(`announcement ${field} must not be empty`)
  }
  return value.trim()
}

export function validateAnnouncementPluginData(value: unknown): asserts value is AnnouncementPluginData {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new PluginValidationError('expected Announcement plugin data to be an object')
  }
...
function validateNewsItem(value: unknown, index: number): void {
  ...
  if (item.sourceType !== ANNOUNCEMENT_SOURCE_TYPE) {
    throw new PluginValidationError('expected official source type', `$.items[${index}].sourceType`)
  }
