import type {
  ...
  RelationCandidate,
  SchemaGapProposal,
  CurationValidationFeedbackCode,
} from "../../skills/knowledge-curation/index.ts";
...
import type {
  ...
  ValidatedKnowledgeChangeSetV03,
} from "../../schemas/knowledge/v03/mutation.ts";
import type { KnowledgeWriteResult } from "../../schemas/knowledge/mutation.ts";
import type {
  ValidationReport,
  ChangeSetValidationResultV03,
} from "../../skills/knowledge-validation/types.ts";
...
export interface SourceProposal {
  ...
  resolution: "source_create" | "source_merge" | "review";
}
export interface ThemeHandling {
  dispositions: Record<
    | "resolved_existing"
    | "resolved_multiple"
    ...
    number
  >;
  reviewItems: Array<{ category: string; mention: string; reason: string }>;
}
export interface SectionBatchSummary {
  sectionCount: number;
  batchCount: number;
  chunkCount: number;
  chunkIds: string[];
...
export interface ModelCallValidationFailure {
  attempt: 1 | 2;
...
export interface ModelCallRecord {
  ...
  succeeded: boolean;
  retryCount: 0 | 1;
  validationFailures?: ModelCallValidationFailure[];
  candidateValidation?: { attempts: CandidateValidationSummary[] };
}
...
export interface ResearchReportKnowledgeIngestionResult {
  ...
  plannedChanges: {
    sourceCreate: string[];
    sourceMerge: string[];
    knowledgeCreate: string[];
    knowledgeUpdate: string[];
    knowledgeSupersede: string[];
    knowledgeSourceMerge: string[];
  };
  committedChanges: {
...
export interface ResearchReportKnowledgeIngestionWorkflowOptions {
  ...
  validation?: {
    validateChangeSet(
      ...
      options?: { mode?: "commit" | "dry_run"; virtualRawRefs?: string[] },
    ): Promise<{
      report: ChangeSetValidationResultV03["report"];
  ...
  writer?: {
    write(
      handle: KnowledgeBaseHandle,
      receipt: ValidatedKnowledgeChangeSetV03,
...
export interface IngestionTrace {
  ...
  resolution: Array<{
    ...
    outcome: "existing_ref" | "new_object_key" | "ambiguous" | "invalid";
    refs: string[];
    objectKey?: string;
