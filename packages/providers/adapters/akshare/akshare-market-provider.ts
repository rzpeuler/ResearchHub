...
export interface AkShareMarketProviderOptions {
...
export class AkShareMarketProvider implements DataProvider<MarketProviderRequest, MarketProviderData> {
  ...
  constructor(options: AkShareMarketProviderOptions = {}) {
  ...
  async fetch(request: MarketProviderRequest) {
    ...
    try {
      ...
      const safeCause = new Error(safeProviderMessage(cause, [this.endpoint]))
      throw new MarketProviderError(
        `akshare-market request failed: ${safeProviderMessage(cause, [this.endpoint])}`,
        safeCause,
      )
    }

    if (!response.ok) {
      throw new MarketProviderError(`akshare-market request failed with HTTP ${response.status}`)
    }
...
async function readJson(response: Response, endpoint: string): Promise<unknown> {
  try {
    ...
    const safeCause = new Error(safeProviderMessage(cause, [endpoint]))
    throw new MarketProviderError(
      `akshare-market response was not valid JSON: ${safeProviderMessage(cause, [endpoint])}`,
      safeCause,
    )
  }
}
...
function normalizeAkShareResponse(
  ...
  clock: () => Date,
): { data: MarketProviderData; metadata: { provider: string; source: string; timestamp: string; quality: 'medium'; confidence: number } } {
  ...
  if (responseSymbol !== canonicalMarketSymbol(requestedSymbol)) {
    throw new MarketProviderError('akshare-market response symbol does not match the request')
  }
...
function readAkShareRow(value: unknown): Record<string, unknown> {
  ...
  if (!isRecord(value)) {
    throw new MarketProviderError('akshare-market response is malformed')
  }
  ...
  throw new MarketProviderError('akshare-market response is malformed: missing row')
}

function readFirstRow(rows: unknown[]): Record<string, unknown> {
  if (rows.length === 0 || !isRecord(rows[0])) {
    throw new MarketProviderError('akshare-market response is empty or malformed')
  }
  return rows[0]
}
