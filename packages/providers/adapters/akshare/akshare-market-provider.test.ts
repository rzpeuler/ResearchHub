...
import { AkShareMarketProvider } from './akshare-market-provider.ts'
...
test('AkShare is explicitly disabled when no bridge endpoint is configured', async () => {
  ...
  await assert.rejects(provider.fetch({ symbol: '600519' }), /akshare-market provider is disabled: endpoint is not configured/)
...
test('AkShare handles HTTP, malformed, empty, and invalid responses', async () => {
  ...
  for (const current of cases) {
    ...
    await assert.rejects(provider.fetch({ symbol: '600519' }), current.expected)
  }
})
