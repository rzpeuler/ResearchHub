...
/** Canonical Information Layer Provider for professional media evidence. */
export class MediaProvider implements DataProvider<MediaProviderRequest, MediaProviderData> {
  ...
  async fetch(request: MediaProviderRequest) {
    ...
    try {
      ...
      throw new MediaProviderError(`media source adapter failed: ${formatCause(cause)}`, cause)
    }
    const items = rawRecords.map((record) => normalizeMediaRecord(record, normalized.symbol, this.issuerToSymbol))
    const timestamp = this.clock()
    if (!(timestamp instanceof Date) || Number.isNaN(timestamp.getTime())) {
      throw new MediaProviderError('media provider clock returned an invalid date')
    }
  ...
  validate(value: unknown): asserts value is MediaProviderData {
    validateMediaProviderData(value)
  }
}

export function normalizeMediaRequest(value: MediaProviderRequest): { symbol: string; limit: number } {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new ProviderValidationError('expected a Media provider request')
  }
...
function normalizeMediaRecord(
  ...
  if (mappedSymbol === undefined) {
    throw new MediaProviderError('media record cannot be mapped to a stock symbol')
  }
  if (mappedSymbol !== requestedSymbol) {
    throw new MediaProviderError('media source symbol does not match the request')
  }
...
export function validateMediaProviderData(value: unknown): asserts value is MediaProviderData {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new ProviderValidationError('expected Media provider data to be an object')
  }
...
function validateMediaItem(value: unknown, index: number): void {
  ...
  if (item.sourceType !== 'media') {
    throw new ProviderValidationError('expected media source type', `$.items[${index}].sourceType`)
  }
  ...
  if (item.metadata === null || typeof item.metadata !== 'object' || Array.isArray(item.metadata)) {
    throw new ProviderValidationError('expected media metadata object', `$.items[${index}].metadata`)
  }
  ...
  if (typeof metadata.tier !== 'string' || !(MEDIA_SOURCE_TIERS as readonly string[]).includes(metadata.tier)) {
    throw new ProviderValidationError('expected tier-1, tier-2, or tier-3', `$.items[${index}].metadata.tier`)
  }
