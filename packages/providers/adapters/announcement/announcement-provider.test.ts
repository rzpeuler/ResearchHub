...
import { AnnouncementProvider, validateAnnouncementProviderData } from './announcement-provider.ts'
import {
...
import { registerAnnouncementProvider } from './news-capability-adapter.ts'
...
test('CNINFO source adapter reports HTTP, JSON, and payload errors', async () => {
  ...
  await assert.rejects(httpFailure.fetch({ symbol: '600519', limit: 1 }), AnnouncementProviderError)

  const invalidJson = new CninfoAnnouncementSourceAdapter({
    transport: { async request() { return new Response('not-json', { status: 200 }) } },
  })
  await assert.rejects(invalidJson.fetch({ symbol: '600519', limit: 1 }), AnnouncementProviderError)
...
test('AnnouncementProvider normalizes official NewsItems and maps issuer-only records', async () => {
  const sourceAdapter = {
  ...
  const provider = new AnnouncementProvider({
    sourceAdapter,
...
test('AnnouncementProvider rejects mismatched or unmappable announcement records', async () => {
  const mismatched = new AnnouncementProvider({
    sourceAdapter: {
      name: 'fixture',
      async fetch() {
        return [{
  ...
  const unmappable = new AnnouncementProvider({
    sourceAdapter: {
      name: 'fixture',
      async fetch() {
        return [{
...
test('announcement-provider registers through Registry and remains callable by unchanged NewsCapability', async () => {
  const registry = new ProviderRegistry()
  const composition = registerAnnouncementProvider(registry, {
    sourceAdapter: {
      name: 'fixture',
      async fetch() {
  ...
  assert.deepEqual(registry.list(), ['announcement-provider'])
  ...
  assert.equal(composition.provider.name, 'announcement-provider')
})

test('Announcement Provider validation rejects non-official and malformed NewsItems', () => {
  assert.throws(
    () => validateAnnouncementProviderData({
      symbol: '600519',
      items: [{
        title: 'Invalid',
        content: 'Invalid',
