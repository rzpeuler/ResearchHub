...
export function assertRecord(value: unknown, message: string): asserts value is Record<string, unknown> {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw new AnnouncementProviderError(message)
  }
}
...
export function readRequiredString(
  ...
  throw new AnnouncementProviderError(`announcement source response is missing ${field}`)
}
...
export function readOptionalConfidence(value: Record<string, unknown>): number | undefined {
  ...
  if (!Number.isFinite(confidence) || confidence < 0 || confidence > 1) {
    throw new AnnouncementProviderError('announcement source confidence must be between 0 and 1')
  }
  return confidence
}
...
export function normalizePublishedAt(value: string): string {
  ...
  if (Number.isNaN(parsed.getTime())) {
    throw new AnnouncementProviderError(`invalid announcement publication time: ${value}`)
  }
...
export function parseRawAnnouncement(value: unknown): RawAnnouncementRecord {
  ...
  if (content === undefined) {
    throw new AnnouncementProviderError('announcement source item is missing content or source URL')
  }
