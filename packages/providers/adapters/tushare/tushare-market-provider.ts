...
import {
  ...
  MarketProviderError,
  normalizeMarketProviderRequest,
  readFiniteNumber,
  readRequiredField,
  requireHttpEndpoint,
  safeProviderMessage,
...
export interface TushareMarketProviderOptions {
...
export class TushareMarketProvider implements DataProvider<MarketProviderRequest, MarketProviderData> {
  ...
  constructor(options: TushareMarketProviderOptions = {}) {
  ...
  async fetch(request: MarketProviderRequest) {
    const normalizedRequest = normalizeMarketProviderRequest(request)
    if (this.token === undefined) {
      throw new MarketProviderError('tushare-market provider is unavailable: TUSHARE_TOKEN is not configured')
    }
    ...
    try {
      ...
      const safeCause = new Error(safeProviderMessage(cause, [this.token, this.endpoint]))
      throw new MarketProviderError(
        `tushare-market request failed: ${safeProviderMessage(cause, [this.token, this.endpoint])}`,
        safeCause,
      )
    }

    if (!response.ok) {
      throw new MarketProviderError(`tushare-market request failed with HTTP ${response.status}`)
    }
...
function toTushareCode(symbol: string): string {
  ...
  if (!/^\d{6}$/.test(symbol)) {
    throw new MarketProviderError(`unsupported A-share symbol: ${symbol}`)
  }
...
async function readJson(response: Response, token: string, endpoint: string): Promise<unknown> {
  try {
    ...
    const safeCause = new Error(safeProviderMessage(cause, [token, endpoint]))
    throw new MarketProviderError(
      `tushare-market response was not valid JSON: ${safeProviderMessage(cause, [token, endpoint])}`,
      safeCause,
    )
  }
}
...
function normalizeTushareResponse(
  ...
  token: string,
  endpoint: string,
): { data: MarketProviderData; metadata: { provider: string; source: string; timestamp: string; quality: 'high'; confidence: number } } {
  if (!isRecord(value) || value.code !== 0) {
    ...
    throw new MarketProviderError(`tushare-market API error: ${safeProviderMessage(message, [token, endpoint])}`)
  }

  if (!isRecord(value.data)) {
    throw new MarketProviderError('tushare-market response is malformed: missing data envelope')
  }
  ...
  if (responseSymbol !== canonicalMarketSymbol(requestedSymbol)) {
    throw new MarketProviderError('tushare-market response symbol does not match the request')
  }

  const change = readRequiredField(row, ['pct_chg', 'change'], 'change')
  const timestamp = timestampFromSource(row.trade_date, clock)
  const data: MarketProviderData = {
...
function readTushareRow(data: Record<string, unknown>): Record<string, unknown> {
  ...
  if (!Array.isArray(items) || items.length === 0) {
    throw new MarketProviderError('tushare-market response is empty')
  }
  ...
  if (!Array.isArray(firstItem) || !Array.isArray(data.fields) || data.fields.length === 0) {
    throw new MarketProviderError('tushare-market response is malformed: invalid row')
  }
  ...
  for (let index = 0; index < data.fields.length; index += 1) {
    ...
    if (typeof field !== 'string' || field.trim().length === 0) {
      throw new MarketProviderError('tushare-market response is malformed: invalid field list')
    }
