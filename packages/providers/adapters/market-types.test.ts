...
import { MarketProviderError, timestampFromSource } from './market-types.ts'
...
test('timestamp normalization rejects impossible calendar dates without JavaScript rollover', () => {
  for (const value of ['2026-02-30', '2026-02-30T09:00:00.000Z', '20260230']) {
    assert.throws(
      ...
      (error: unknown) => error instanceof MarketProviderError
        && /invalid provider timestamp/.test(error.message),
    )
  }
})
