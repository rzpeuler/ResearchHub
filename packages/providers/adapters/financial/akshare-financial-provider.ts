...
export interface AkShareFinancialProviderOptions {
...
export class AkShareFinancialProvider implements FinancialProvider {
  ...
  constructor(options: AkShareFinancialProviderOptions = {}) {
  ...
  async fetch(request: Parameters<FinancialProvider['fetch']>[0]) {
    ...
    try {
      ...
      if (cause instanceof FinancialProviderError) throw cause
      throw new FinancialProviderError(
        `${this.name} request failed: ${safeProviderMessage(cause, this.endpoint)}`,
        cause,
      )
    }
    ...
    if (data.symbol !== normalized.symbol || data.statements.length !== normalized.statementTypes.length) {
      throw new FinancialProviderError('akshare-financial response did not contain the requested statements')
    }
    const firstSource = data.statements[0]?.source
    if (firstSource === undefined) {
      throw new FinancialProviderError('akshare-financial response did not contain source metadata')
    }
...
function normalizeAkShareRow(
  ...
  if (symbol !== requestedSymbol) {
    throw new FinancialProviderError('akshare-financial response symbol does not match the request')
  }
...
function statementTypeFor(row: Record<string, unknown>): FinancialStatementType {
  ...
  throw new FinancialProviderError('akshare-financial response is missing statement type')
}
