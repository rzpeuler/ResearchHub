...
import { requireHttpEndpoint, safeProviderMessage } from '../market-types.ts'
...
import type { FinancialData, FinancialProvider, FinancialStatementType } from './types.ts'

export interface TushareFinancialProviderOptions {
...
export class TushareFinancialProvider implements FinancialProvider {
  ...
  constructor(options: TushareFinancialProviderOptions = {}) {
  ...
  async fetch(request: Parameters<FinancialProvider['fetch']>[0]) {
    const normalized = normalizeFinancialRequest(request)
    if (this.token === undefined) {
      throw new FinancialProviderError('tushare-financial provider is unavailable: TUSHARE_TOKEN is not configured')
    }
    ...
    for (const statementType of normalized.statementTypes) {
      ...
      if (latestRow === undefined) {
        throw new FinancialProviderError(`${this.name} response is empty for ${statementType}`)
      }
    ...
    if (data.symbol !== normalized.symbol || data.statements.length !== normalized.statementTypes.length) {
      throw new FinancialProviderError('tushare-financial response did not contain the requested statements')
    }
    const firstSource = data.statements[0]?.source
    if (firstSource === undefined) {
      throw new FinancialProviderError('tushare-financial response did not contain source metadata')
    }
  ...
  private async requestStatement(symbol: string, statementType: FinancialStatementType): Promise<unknown> {
    ...
    try {
      ...
      if (cause instanceof FinancialProviderError) {
        throw cause
      }
      throw new FinancialProviderError(
        `${this.name} request failed: ${safeProviderMessage(cause, [this.token, this.endpoint])}`,
        cause,
      )
    }
  }
...
function normalizeTushareRow(
  ...
  if (toSixDigitSymbol(responseSymbol) !== symbol) {
    throw new FinancialProviderError('tushare-financial response symbol does not match the request')
  }
  const retrievedAt = clock()
  if (!(retrievedAt instanceof Date) || Number.isNaN(retrievedAt.getTime())) {
    throw new FinancialProviderError('invalid provider clock timestamp')
  }
  return {
    statementType,
    symbol,
...
function toTushareCode(symbol: string): string {
  ...
  throw new FinancialProviderError(`unsupported A-share symbol: ${symbol}`)
}
