...
async function main(): Promise<void> {
  ...
  try {
    ...
    ;(evidence.primary as JsonRecord).batching = planned
    ...
    primaryAccounting.changeSetIngestionContextModelCalls = primary.status === 'blocked' ? null : primaryLog.modelCalls
    primaryAccounting.changeSetEqualityApplicable = primary.status !== 'blocked'
    primaryAccounting.changeSetMatches = primary.status === 'blocked' ? null : primaryLog.modelCalls === primaryAccounting.physicalModelInvocations
