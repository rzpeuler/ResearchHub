...
export function buildReconciliationBoundary(primary: { referenceResolutionReached: boolean; referenceResolution: { existing_ref: number; new_object_key: number; ambiguous: number; invalid: number }; reconciliationPlanningReached: boolean; reconciliation: { groups: number }; }, logicalCalls: number, physicalCalls: number): ReconciliationBoundaryEvidence {
  if (!primary.referenceResolutionReached) {
    return { status: "not_reached", existingRefCandidates: null, newObjectKeyCandidates: null, ambiguousCandidates: null, invalidCandidates: null, reconciliationPlanningReached: false, reconciliationGroups: null, logicalCalls, physicalCalls };
  }
  const boundary = { status: "reached_and_failed" as ReconciliationBoundaryStatus, existingRefCandidates: primary.referenceResolution.existing_ref, newObjectKeyCandidates: primary.referenceResolution.new_object_key, ambiguousCandidates: primary.referenceResolution.ambiguous, invalidCandidates: primary.referenceResolution.invalid, reconciliationPlanningReached: primary.reconciliationPlanningReached, reconciliationGroups: primary.reconciliationPlanningReached ? primary.reconciliation.groups : null, logicalCalls, physicalCalls };
  boundary.status = c14FreshKbReconciliationBoundaryPasses(boundary) ? "reached_and_passed" : "reached_and_failed";
  return boundary;
}
export function c14FreshKbReconciliationBoundaryPasses(boundary: Pick<ReconciliationBoundaryEvidence, "existingRefCandidates" | "reconciliationPlanningReached" | "reconciliationGroups" | "logicalCalls" | "physicalCalls">): boolean {
  return boundary.reconciliationPlanningReached === true && boundary.existingRefCandidates === 0 && boundary.reconciliationGroups === 0 && boundary.logicalCalls === 0 && boundary.physicalCalls === 0;
}
...
export type RetryAttributionCause = "completion_failure" | "candidate_validation_failure" | "candidate_set_exhausted" | "unattributed_retry";
export interface RetryAttribution {
  retryCount: number;
  causes: Array<{ fromAttempt: number; toAttempt: number; code: string | null; cause: RetryAttributionCause }>;
  partialCandidateAttemptTriggeredRetry: boolean;
}

function candidateValidationAttemptNumbers(batch: JsonRecord): number[] {
  const attempts = Array.isArray(batch.validationAttempts) ? batch.validationAttempts.filter(isRecord) : [];
  const failures = Array.isArray(batch.validationFailures) ? batch.validationFailures.filter(isRecord) : [];
  const physicalAttempts = Math.max(1, numberOrZero(batch.physicalAttempts), numberOrZero(batch.retryCount) + 1);
  let nextAttempt = 1;
  return attempts.map((summary) => {
    const explicitAttempt = numberOrZero(summary.attempt);
    if (explicitAttempt > 0) {
      nextAttempt = Math.max(nextAttempt, explicitAttempt + 1);
      return explicitAttempt;
    }
    while (nextAttempt <= physicalAttempts) {
      const failure = failures.find((item) => numberOrZero(item.attempt) === nextAttempt);
      const allCandidatesRejected = totalCount(summary.accepted) === 0 && totalCount(summary.rejected) > 0;
      if (!failure || (failure.code === "candidate_set_exhausted" && allCandidatesRejected)) break;
      nextAttempt += 1;
    }
    const assigned = Math.min(nextAttempt, physicalAttempts);
    nextAttempt = assigned + 1;
    return assigned;
  });
}

function retryCauseForCode(code: string): RetryAttributionCause {
  if (code === "candidate_set_exhausted") return "candidate_set_exhausted";
  if (code === "invalid_model_output") return "completion_failure";
  if (code === "invalid_reference" || code === "invalid_semantics" || code === "invalid_confidence" || code === "ungrounded_candidate") return "candidate_validation_failure";
  return "unattributed_retry";
}

export function buildRetryAttribution(batch: JsonRecord): RetryAttribution {
  const retryCount = Math.max(0, numberOrZero(batch.retryCount));
  const physicalAttempts = Math.max(1, numberOrZero(batch.physicalAttempts), retryCount + 1);
  const failures = Array.isArray(batch.validationFailures) ? batch.validationFailures.filter(isRecord) : [];
  const validationAttempts = Array.isArray(batch.validationAttempts) ? batch.validationAttempts.filter(isRecord) : [];
  const candidateAttempts = candidateValidationAttemptNumbers(batch);
  const partialAttempts = new Set(candidateAttempts.filter((_, index) => {
    const summary = validationAttempts[index];
    return Boolean(summary && totalCount(summary.accepted) > 0 && totalCount(summary.rejected) > 0);
  }));
  const causes: RetryAttribution["causes"] = [];
  let partialCandidateAttemptTriggeredRetry = false;
  for (let fromAttempt = 1; fromAttempt < physicalAttempts; fromAttempt += 1) {
    const failure = failures.find((item) => numberOrZero(item.attempt) === fromAttempt);
    if (failure && typeof failure.code === "string") {
      causes.push({ fromAttempt, toAttempt: fromAttempt + 1, code: failure.code, cause: retryCauseForCode(failure.code) });
      continue;
    }
    if (partialAttempts.has(fromAttempt)) partialCandidateAttemptTriggeredRetry = true;
    causes.push({ fromAttempt, toAttempt: fromAttempt + 1, code: null, cause: "unattributed_retry" });
  }
  return { retryCount, causes, partialCandidateAttemptTriggeredRetry };
}
...
function primaryEvidence(result: ResearchReportKnowledgeIngestionResult, model: R9RecordingModel, runtime: FullValidationObservingRuntime, writerInvocations: number, planned: JsonRecord): JsonRecord {
  ...
  const batches: JsonRecord[] = result.batches.batches.map((batch) => {
    ...
    const validationAttemptNumbers = candidateValidationAttemptNumbers({
      physicalAttempts: calls.length,
      retryCount: logical?.retryCount ?? 0,
      validationAttempts: attempts,
      validationFailures: logical?.validationFailures ?? [],
    });
    ...
    return {
      ...
      validationAttempts: attempts.map((attempt, index) => sanitizeCandidateSummary(attempt, validationAttemptNumbers[index])),
  ...
  return {
    ...
    reconciliationPlanningReached: result.reconciliationPlanningReached,
    referenceResolution: result.referenceResolutionReached ? result.referenceResolution : null,
    reconciliation: { ...result.reconciliation, groups: result.reconciliationPlanningReached ? result.reconciliation.groups : null, exactlyOnce: Object.values(result.reconciliation.decisions).reduce((sum, count) => sum + count, 0) === result.reconciliation.candidates },
...
function summarizeResult(result: ResearchReportKnowledgeIngestionResult): JsonRecord { return { status: result.status, revision: { before: result.baseRevision, after: result.finalRevision }, raw: { persisted: result.raw.persisted, created: result.raw.created, reused: result.raw.reused }, sourceResolution: result.source?.resolution ?? null, batching: result.batches, extraction: result.extraction, consolidation: result.consolidation, referenceResolutionReached: result.referenceResolutionReached, reconciliationPlanningReached: result.reconciliationPlanningReached, referenceResolution: result.referenceResolutionReached ? result.referenceResolution : null, reconciliation: { ...result.reconciliation, groups: result.reconciliationPlanningReached ? result.reconciliation.groups : null }, schemaGapCount: result.schemaGaps.length, reviewItemCount: result.reviewItems.length, plannedChangeCounts: changeCounts(result.plannedChanges), committedChanges: result.committedChanges, validation: result.validation?.status ?? null, modelCalls: result.modelCalls.length, errorCodes: result.errors.map((error) => error.code) }; }
