...
type R9ModelCall = JsonRecord & { operation: string; batchId?: string; physicalAttempt: number; delegatedToProvider: boolean };
...
class R9RecordingModel implements KnowledgeCurationModel {
  ...
  async invoke(request: KnowledgeCurationModelRequest): Promise<unknown> {
    ...
    const key = `${request.operation}|${batchId ?? ""}`;
    ...
    const call: R9ModelCall = {
      ...
      input: request.operation === "extractKnowledge" ? extractionInputObservation(input) : inputObservation(input),
